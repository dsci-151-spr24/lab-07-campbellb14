...
***This data was from an observational study, there were no assigned treatment groups.***
...
***There are `r ncol(Whickham)` variables in the dataset. Both `outcome` and `smoker` are character data that are two level factors and `age` is numerical data.***
...
```
```{r}
ggplot(data=Whickham, aes(y=smoker, fill=outcome)) +
  geom_bar() +
...
***In the plot, it appears smokers have a higher probability of being dead in each age category.***
